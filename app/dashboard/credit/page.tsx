...
import { Shield, TrendingUp, CheckCircle, AlertCircle, DollarSign, Loader2 } from "lucide-react"
...
export default function CreditScorePage() {
  ...
  const getFactorStatus = (score: number) => {
    ...
    if (score >= 75) return { icon: CheckCircle, color: "text-blue-600", label: "Good" }
    if (score >= 60) return { icon: AlertCircle, color: "text-yellow-600", label: "Fair" }
  ...
  return (
    <DashboardLayout user={user}>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold font-serif">Your Financial Passport</h1>
          <p className="text-muted-foreground">Build your credit score through consistent gig work</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {Object.entries(creditScore.factors).map(([key, value]: [string, any]) => {
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            {creditScore.loanEligibility.length === 0 ? (
              <p className="text-center text-muted-foreground py-8">Complete more gigs to unlock loan eligibility</p>
            ) : (
              creditScore.loanEligibility.map((loan: any, index: number) => (
                <div key={index} className="flex items-center justify-between p-4 border border-border rounded-lg">
                  <div>
                    <p className="font-semibold">{loan.lender}</p>
                    <p className="text-sm text-muted-foreground">Interest Rate: {loan.interestRate}% per month</p>
                  </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Timely payments from clients improve your payment history score
                </p>
              </div>
            </div>
          </div>
